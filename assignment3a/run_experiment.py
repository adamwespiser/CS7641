...
if __name__ == '__main__':
    ...
    dataset2_details = {
            ...
            'best_nn_params': {'NN__activation': ['logistic'], 'NN__alpha': [0.001],
                               'NN__hidden_layer_sizes': [(38, 38)], 'NN__learning_rate_init': [0.128]}
        }
    dataset1_details = {
              'data': loader.WineQualityUniq(verbose=verbose, seed=seed),
            'name': 'wine-qual',
            'readable_name': 'Wine Data (Quality)',
            'best_nn_params': {'NN__activation': ['tanh'], 'NN__alpha': [0.1],
                               'NN__hidden_layer_sizes': [(12,12)], 'NN__learning_rate_init': [0.064]}
    }
    if args.dataset1:
